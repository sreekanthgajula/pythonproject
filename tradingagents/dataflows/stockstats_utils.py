...
def load_ohlcv(symbol: str, curr_date: str) -> pd.DataFrame:
    ...
    vendor = config.get("data_vendors", {}).get("technical_indicators", "yfinance")
    vendor_label = "Zerodha" if vendor == "zerodha" else "YFin"
    ...
    if data is None:
        if vendor == "zerodha":
            from .zerodha import get_zerodha_stock_df
            downloaded = get_zerodha_stock_df(symbol, start_str, end_str)
            downloaded = _ensure_date_column(downloaded)
            if downloaded.empty or "Close" not in downloaded.columns:
                raise NoMarketDataError(
                    symbol, symbol, "Zerodha returned no rows"
                )
        else:
