...
class DataflowsConfigIsolationTests(unittest.TestCase):
    ...
    def test_partial_nested_update_preserves_existing_defaults(self):
        ...
        fresh = get_config()
        self.assertEqual(fresh["data_vendors"]["core_stock_apis"], "alpha_vantage")
        self.assertEqual(fresh["data_vendors"]["technical_indicators"], default_config.DEFAULT_CONFIG["data_vendors"]["technical_indicators"])
        self.assertEqual(fresh["data_vendors"]["fundamental_data"], default_config.DEFAULT_CONFIG["data_vendors"]["fundamental_data"])
        self.assertEqual(fresh["data_vendors"]["news_data"], default_config.DEFAULT_CONFIG["data_vendors"]["news_data"])
